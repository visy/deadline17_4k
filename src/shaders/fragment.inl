 ...
 "{"
   ...
   "if(-abs(v.y)+.8>0)"
     "{"
       ...
       "float l=m*.1,h=m*9.9;"
       "if(s==2)"
         "m-=5,h=1000-((m-54)*.5+m*.15),F=(m-54)*.04,l=5+(v.x-v.y)*.1;"
       "if(s==1&&m>15&&m<20)"
         "h+=d(v*10)*sin((m-30)*1.9)*(1+(m-15)*.1);"
       "if(m>90)"
         "C-=(m-90)*.1;"
       "v*=F;"
       "v*=C;"
       "F=clamp(F,0,1);"
       "if(s==0)"
         "h=2779,c=5-m*.1,y=.15,l=12-m*.41,Z=distance(vec2(0),vec2(v.x,v.y))*.4;"
       "else"
         " l=l*min(max(m-4.25,0),1)+min(max(m-15,0),10)*8*max(1-max(m-21,0)*.3,0);"
       "if(m>49)"
         "h-=cos(d(v*cos(h*.01+v.x*.1)*8)*mod(m*.01,1)+h);"
       "vec3 o=vec3(0,1,h),Y=normalize(vec3(0,1,h+1)-o),X=normalize(vec3(Y.z,0,-Y.x)),W=normalize(cross(Y,X)),V=o,U=normalize(Y+l*v.x*X+l*v.y*W);"
       "vec2 T=a(V,U,350);"
       "if(s==1)"
       ...
       "if(s==2)"
         "S=vec3(3-T.x*.05);"
       "vec3 R=V+U*T.x,Q=vec3(y,0,0),P=vec3(u(R+Q.xyy)-u(R-Q.xyy),u(R+Q.yxy)-u(R-Q.yxy),u(R+Q.yyx)-u(R-Q.yyx)),O=normalize(P);"
       "g++;"
       "vec3 N=reflect(U,O),M=normalize(vec3(sin(T.x*.1),.3,-1+s));"
       "float L=dot(M,U),K=dot(M,O),J=dot(O,U),I=L-K*J,H=.3*s*.3*s,G=.6*s*max(0,K)*(1+H*(.6*s/(H+.13)+.5/(H+.33))+.45*H/(H+.09)*I/mix(1,max(K,J),step(0,I)))/3.14159;"
       "vec3 E=mix(vec3(G),S*(G*vec3(1)+vec3(1))*Z+smoothstep(-.1,.9,N.y)*.2+pow(clamp(dot(N,M),0,1),12)*.6,F)*C;"
       "i=vec4(1.2*E/(1+min(max(w-30,0),1)*(-1+min(a(R+b*U*max(32-m/4,7)/(1+T.x*.01),U,32).x+t(e*1e+06)*2,7))/3),T.x/128);"
     "}"
 "}";
